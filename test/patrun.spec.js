...
describe('patrun', function(){
  ...
  it('add', function(done) {
    ...
    expect( JSON.stringify(r.list()) ).to.equal('[{"match":{"a":"1"},"data":"r1"}]')
    ...
    expect( JSON.stringify(r.list()) ).to.equal('[{"match":{"a":"1","b":"3"},"data":"r2"},{"match":{"a":"1","c":"2"},"data":"r1"}]')
    done()
  })
  ...
  function listtest(mode) {
    return function(done) {
      ...
      expect( '[{"match":{"p1":"v1"} ).to.equal( "data":"r0"}]',JSON.stringify(found))


      found = rt1.list({p1:'v1',p2:'*'})
      expect( '[{"match":{"p1":"v1" ).to.equal( "p2":"v2a"},"data":"r1"},{"match":{"p1":"v1","p2":"v2b"},"data":"r2"}]',JSON.stringify(found))


      rt1.add( {p1:'v1',p2:'v2c',p3:'v3a'}, 'r3a' )
      rt1.add( {p1:'v1',p2:'v2d',p3:'v3b'}, 'r3b' )
      found = rt1.list({p1:'v1',p2:'*',p3:'v3a'})
      expect( '[{"match":{"p1":"v1" ).to.equal( "p2":"v2c","p3":"v3a"},"data":"r3a"}]',JSON.stringify(found))
  ...
  it('null-undef-nan', function(done){
    ...
    rt1.add( {p99:'v99'}, 'r99' )
    expect(  '{"d":"r2" ).to.equal( "k":"p99","v":{"v99":{"d":"r99"}}}', rt1.toJSON() )
    done()
  })
  ...
  it('all', function(done){
    ...
    expect( JSON.stringify(p.list()) ).to.equal('[{"match":{"a":"1"},"data":"X"},{"match":{"b":"2"},"data":"Y"}]')
    done()
  })
  ...
  it('list-any', function(done){
    ...
    expect( JSON.stringify(p1.list({a:'*'})) ).to.equal( '['+[mA,mB,mC]+']' )
    expect( JSON.stringify(p1.list({b:'*'})) ).to.equal( '['+[mB,mC]+']' )
    expect( JSON.stringify(p1.list({c:'*'})) ).to.equal( '['+[mC]+']' )

    expect( JSON.stringify(p1.list({a:1,b:2}) ) ).to.equal( '['+[mB,mC]+']' )
    expect( JSON.stringify(p1.list({a:1,b:'*'}) ) ).to.equal( '['+[mB,mC]+']' )
    expect( JSON.stringify(p1.list({a:1,b:'*',c:3}) ) ).to.equal( '['+[mC]+']' )
    expect( JSON.stringify(p1.list({a:1,b:'*',c:'*'}) ) ).to.equal( '['+[mC]+']' )
    ...
    expect( JSON.stringify(p1.list({a:1})) ).to.equal( '['+[mA,mB,mC,mD]+']' )
    ...
    expect( JSON.stringify(p1.list()) ).to.equal( '['+[mA,mB,mC,mCC,mD]+']' )
    expect( JSON.stringify(p1.list({a:1})) ).to.equal( '['+[mA,mB,mC,mCC,mD]+']' )
    ...
    expect( JSON.stringify(p1.list({a:1,c:'*'})) ).to.equal( '['+[mC,mCC]+']' )
    ...
    expect( JSON.stringify(p1.list({a:1,b:2},true) ) ).to.equal( '['+[mB]+']' )
    expect( JSON.stringify(p1.list({a:1,b:'*'},true) ) ).to.equal( '['+[mB]+']' )
    expect( JSON.stringify(p1.list({a:1,c:3},true) ) ).to.equal( '[]' )
    expect( JSON.stringify(p1.list({a:1,c:33},true) ) ).to.equal( '['+[mCC]+']' )
    expect( JSON.stringify(p1.list({a:1,c:'*'},true) ) ).to.equal( '['+[mCC]+']' )
    done()
  })
